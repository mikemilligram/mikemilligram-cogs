...
class LoL(commands.Cog):
    ...
    champs = app_commands.Group(name="champs", description="League of Legends commands")

      
    @app_commands.checks.has_permissions(administrator=True)
    @champs.command(name='update')
    ...
    @champs.command(name='reset')
    async def reset_champs(self, interaction: discord.Interaction):
    ...
    @champs.command(name='remove')
    async def remove_champ(self, interaction: discord.Interaction, champ: str):
    ...
    @champs.command(name='add')
    async def add_champ(self, interaction: discord.Interaction, champ: str):
        champ_list = await self.config.member(interaction.user).champs()
    ...
    @champs.command(name='random')
    async def random_champ(self, interaction: discord.Interaction):
        champ_list = await self.config.member(interaction.user).champs()
        if champ_list:
